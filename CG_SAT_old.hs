...
import CG_base hiding ( name, isAmbig )
import CG_data
import Data.List
import Data.Maybe
    ...
    concat `fmap` sequence
          [ do let condLits = map getLit cToks
               ...
               case (positive, any isCautious cToks) of
                 ...
                 (Neg,False) -> do c_NOT1 <- orl s "--NOT_1" condLits 
                                   return $ c_NOT1:map neg nomatch 
                 --NOT 1C
                 (Neg, True) -> (:[]) `liftM` orl s "--NOT_1C" condLits
            | (c@(C _pos (positive,ctags)), cToks) <- ctx 
            , let noMatch t = (if positive==Pos then not else id) $ 
...
getContext ana allToks (cond@(C position ts@(positive,ctags)):cs) = --trace ("getContext.ana: " ++ show ana ++ ", cond:" ++ show cond ++ ", result:" ++ show result) $ 
 ...
 where 
  ...
  match :: Token -> Bool
  match tok = (if positive==Pos then id else not) $ tagsMatchRule tok (toTags ctags)
  ...
  exactly n token = let potentialToks = 
                    ...
                    in if null potentialToks && positive==Neg
                         then [dummyTok]
                         else potentialToks
