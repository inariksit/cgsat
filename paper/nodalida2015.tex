...
ordering.
The encoding in logic captures richer dependencies between the tags:
a rule is allowed to take action also concerning the words in its
context. This makes the number of the rules potentially smaller. 
...
There are no inputs that satisfy the requirement with first order,
but several that work with the second, for instance
...
We experimented with both ordered and unordered rules, and found the ordered
scheme to work better with previously written grammars.
The evaluation shows that SAT-CG isn't a match for
As future work, we 
As a complementary approach, we intend to concentrate on grammar analysis
