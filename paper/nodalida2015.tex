...
As described in the previous section, the dependencies between tags
are stronger than in standard CG, so we get a unique solution with \texttt{det n adj}.
...
traditional CG is written in a more imperative way, whereas SAT-CG
...
It is also slightly better for the performance, because the maximisation
...
writer, since it removes the need to estimate the best sequence of
rule applications and whether to make the rules cautious. On the other
hand, it is also less transparent, and might not scale up for larger grammars.
