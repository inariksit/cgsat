...
young formalism. It belongs to the reductionist family of parsing
strategies, along with Finite-State Intersection Grammar (FSIG) \cite{koskenniemi90}.
In this work, we use a SAT solver to implement a version of CG that is
roughly compatible with CG-2.
...
Our work is inspired by \cite{lager98}, which presents constraint
...
\cite{lindberg_eineborg98ilp} and \cite{asfrent14} use Inductive Logic Programming to learn CG rules from a tagged corpus.
...
See also \cite{lager01transformation} for discussion of ordering of
the CG rules.
