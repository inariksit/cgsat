...
However, our rule is not the only way to reach this form: since
disjunction doesn't care about order, we notice that also
...
In SAT-CG, the notion of cautious context is irrelevant.
When the rules are applied to the text, they are translated into
implications, such as \\ 
 \emph{casa}\texttt{<n>} $\Rightarrow$ $\neg$\emph{la}\texttt{<prn>},
 which reads ``if the \texttt{n} reading for \emph{casa} is true, then
 discard the \texttt{prn} reading for \emph{la}''. Each rule generates
 a number of these implications, and the SAT solver tries to find a model that will satisfy them. 
...
depends of the order of the two rules; if the \texttt{det} reading of \emph{la} is removed first, then we are stuck with that choice.
...
The SAT-CG implementation is ideal for this kind of corpus-based grammar
...
For future work, we intend to apply the SAT solver in testing for
conflicts without a corpus. Let us illustrate the idea with the rule
...
after applying the first rule, can be modified by the second rule.

Implementing the design on a larger scale is left for future work.
...
Yli-Jyrä for comments and suggestions; also everyone at the CG
workshop who participated in the discussion.
