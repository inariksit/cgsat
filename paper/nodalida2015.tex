...
As hinted by the previous property, SAT solver does not need a fixed
order of the rules.
...
We kept only SELECT and REMOVE rules, which left us 261 rules.
...
We also wanted to test the hypothesis that SAT-CG works better with
small amount of rules. We took the base grammar and started
...
of only 19 rules, unordered SAT-CG achieves 85.3 \% and VISL CG-3 85.9
...
especially finding conflicts in grammars.
