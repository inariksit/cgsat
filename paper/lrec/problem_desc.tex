...
\end{itemize}
$R$ can be one rule or a set of rules: for instance, if one rule removes a verb in
...
or it does not fire. In case the rule does not fire, we have again two
options: either its conditions are not met,
or its target is the only remaining analysis. This results in a complex table
