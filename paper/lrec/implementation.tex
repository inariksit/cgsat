...
The shortcut works most of the time, but if only take the readings
from the grammar and ignore the lexicon, it is possible to
miss some cases: e.g. \texttt{SELECT Pron + Rel IF (0 Nom)} 
may assume ``pron rel nom'' in one reading, but this method only gives
