...
When we added ambiguity class constraints, we found three more true conflicts, all internal.
Interestingly, the version \emph{with} ambiguity classes fails to detect an interaction conflict, which the simpler version reports, because one of the rules was first detected as internal conflict.
...
One of the conflicts that was only caught by the ambiguity class constraints had the condition \texttt{IF (1 Comma) (..) (1 CnjCoo)}. The additional constraints correctly prevent the SAT-solver to make comma ambiguous with anything else.
